...
def read_explore_csv(credentials: HTTPBasicCredentials = Depends(security)):
    ...
    df = pd.read_csv(r'https://github.com/javiersan48/OpencClassroom-Project/blob/master/P7_Data_Dashboard_explore.csv') 
    df = df.fillna('')               
...
def read_predict_csv(credentials: HTTPBasicCredentials = Depends(security)):
    verify_credentials(credentials)
    df = pd.read_csv(r'https://github.com/javiersan48/OpencClassroom-Project/blob/master/P7_Data_Dashboard_predict.csv')
    df = df.fillna('')
    data = df.to_dict(orient="records")
    return data
...
async def get_logo_image(credentials: HTTPBasicCredentials = Depends(security)):
    verify_credentials(credentials)
    image_path = r"https://github.com/javiersan48/OpencClassroom-Project/blob/master/logo_pret_a_depenser.PNG"
...
async def get_cover_image(credentials: HTTPBasicCredentials = Depends(security)):
    verify_credentials(credentials)
    image_path = r"https://github.com/javiersan48/OpencClassroom-Project/blob/master/PhotoJ.PNG"
    image = Image.open(image_path)
    image_bytes = BytesIO()
    image.save(image_bytes, format="PNG")
    image_bytes.seek(0)
...
async def get_lgbm_image(credentials: HTTPBasicCredentials = Depends(security)):
    verify_credentials(credentials)
    image_path = r"https://github.com/javiersan48/OpencClassroom-Project/blob/master/LightGBM.png"
    image = Image.open(image_path)
    image_bytes = BytesIO()
    image.save(image_bytes, format="PNG")
    image_bytes.seek(0)
...
async def get_credit_image(credentials: HTTPBasicCredentials = Depends(security)):
    verify_credentials(credentials)
    image_path = r"https://github.com/javiersan48/OpencClassroom-Project/blob/master/dreamcredit.PNG"
    image = Image.open(image_path)
    image_bytes = BytesIO()
    image.save(image_bytes, format="PNG")
    image_bytes.seek(0)
...
model_path = r'https://github.com/javiersan48/OpencClassroom-Project/blob/master/LightGBM_with_threshold.pkl'
with open(model_path, 'rb') as f:
    model_with_threshold = pickle.load(f)
    model = model_with_threshold['model']

df2 = pd.read_csv(r'https://github.com/javiersan48/OpencClassroom-Project/blob/master/P7_Data_Dashboard_predict.csv')
...
async def get_data_drift(credentials: HTTPBasicCredentials = Depends(security)):
    verify_credentials(credentials)
    with open(r"https://github.com/javiersan48/OpencClassroom-Project/blob/master/DataDrift.html", "r", encoding="utf-8") as report:
